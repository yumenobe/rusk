...
document.querySelector('#add-routine').addEventListener('click',()=>{const start=document.querySelector('#default-start').value || '09:00',end=document.querySelector('#default-end').value || '10:00';routines.unshift({id:crypto.randomUUID(),title:'新しいルーティン',genre:'personal',period:'morning',weekdayStart:start,weekdayEnd:end,weekendStart:start,weekendEnd:end,enabled:true});renderRoutines();const newRoutine=holder.querySelector('.routine-editor');newRoutine?.scrollIntoView({behavior:'smooth',block:'center'});const newTitle=newRoutine?.querySelector('.routine-title');newTitle?.focus();newTitle?.select();});
